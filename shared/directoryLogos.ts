...
const STEM_BY_NORMALIZED: Record<string, string> = {
  thenestlounge: "place-thenestlounge",
  greenhousehaircollective: "place-greenhousehaircollective",
  ringdingding: "place-ringdingding",
  theimperialsovereignrosecourtoforegon: "place-theimperialsovereignrosecourtoforegon",
  badgirlspdx: "place-badgirlspdx",
  tranzguyspdx: "place-tranzguyspdx",
  lavenderleague: "place-lavenderleague",
  pdxgaymers: "place-pdxgaymers",
  sankofacollective: "place-sankofacollective",
  thesecretwarehouse: "place-thesecretwarehouse",
  fantasy: "Fantasy_Adults_Only",
  inbundancewealthmanagement: "place-inbundancewealthmanagement",
  gaylabration: "place-gaylabration",
  futureprairie: "place-futureprairie",
  multnomahcountyculturalcoalition: "place-multnomahcountyculturalcoalition",
  backstage: "place-backstage",
  pdxbackstage: "place-pdxbackstage",
  scoreportland: "place-scoreportland",
...
export function resolveDirectoryLogo(
  ...
  // added after the hand-authored map once the pack builder has run.
  if (norm) return withLogoCacheBust(`/directory-logos/place-${norm}.png`);
  if (imageUrl && imageUrl.trim()) {
    const url = imageUrl.trim();
    return url.startsWith("/directory-logos/") ? withLogoCacheBust(url) : null;
  }
  return null;
}
...
export function hasDirectoryLogo(name: string, imageUrl?: string | null): boolean {
  const norm = normalizeDirectoryName(name);
  if (STEM_BY_NORMALIZED[norm] || norm) return true;
  return Boolean(imageUrl?.trim().startsWith("/directory-logos/"));
}
