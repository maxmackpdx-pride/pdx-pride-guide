...
if (blockedGate.ok) assert.equal(blockedGate.publishable, true, "unpassed mistake tests are reported, not publication blockers");
...
if (approvedGate.ok) assert.equal(approvedGate.publishable, true, "recorded older conflicts do not veto the latest verified fact");
