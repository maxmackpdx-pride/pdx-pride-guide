...
test('every destination has unique identity and shipped artwork',async()=>{
 assert.equal(places.length,362);assert.equal(new Set(places.map(p=>p.id)).size,places.length);
...
test('winter catalog contains eight resorts per state and all official report links',async()=>{
 const added=places.filter(p=>p.kind==='winter');assert.equal(added.length,16);
 for(const state of ['OR','WA'])assert.equal(added.filter(p=>p.state===state).length,8);
 for(const p of added){assert.ok(p.winter.reportUrl.startsWith('https://'));assert.ok(p.winter.passUrl.startsWith('https://'));assert.ok(p.lat>=42&&p.lat<49&&p.lng>-125&&p.lng<-116);}
});
