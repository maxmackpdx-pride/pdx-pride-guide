...
- The homepage NEXT preview uses one six-card set. Three cards enter from the left and three from the right at the same time, collide at center, overshoot, rebound once, make one short restrained wiggle, settle into a centered overlapping stack, and stay in that final composition.
...
- The full NEXT section uses a denser clipped topographic/trail-map blueprint field with oversized ghosts derived from the six approved NEXT product-logo families. Keep map and logo forms monochrome, low opacity, non-parallax, and behind the heading and card stack.
