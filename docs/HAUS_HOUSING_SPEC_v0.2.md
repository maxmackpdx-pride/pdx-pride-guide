...
- **Verification is mandatory and free — safety is never for sale.** Every property manager on the board is verified. There is **no unverified tier and no "list first, verify later"** — an unverified landlord simply isn't on the board. And because verification is a **safety gate**, it is **never gated behind payment, and never sold as a badge**: you can't buy your way onto the board, you earn it. Whatever gets monetized later is the *service on top* (distribution, the scraper, analytics, featured reach), never the verification or the safety layer itself.
...
**The "Managed Property" entry point works like promoter sign-up.** The "Managed Property" button adapts to who taps it:

- **Already an approved property manager →** it opens their **property-manager page**: their listings (which they can edit — details only), plus access to **their own QSearch custom trusted scan tool** — the scraper tied to their rental site, scoped to just their properties, so they can run or preview a scan and see what's been ingested.
- **Not approved yet →** it opens a **sign-up application to become a property manager**, exactly like the existing promoter application. Submitted applications **land in the owner's inbox** for review, and since only the owner can add property managers, the owner is who approves them. Approval flips the account to a verified PM, which is also what switches on their scraper (above).

**Safeguards, freshness, and fairness (required for this type, not optional).** These listings are third-party landlord copy pulled in automatically, so they carry risks the peer types don't. A few guardrails are mandatory:

- **Fair-housing scan on ingest.** Rental ads are exactly where discriminatory language shows up ("no kids," "perfect for young professionals," source-of-income exclusions — and in Oregon/Portland, refusing Section 8 is illegal). Every scraped listing runs through a moderation pass that **flags suspect phrases to the owner's inbox before it goes live**, rather than auto-publishing raw. Republishing a discriminatory ad under ZAYLIST's brand is a real liability.
- **Scam / legitimacy verification.** Fake property managers are the top rental-fraud vector. The owner approves each PM, but on real signal: **domain ownership of the rental site, a business license, and a match to the directory business record.** Decide the required proof at application time (§ entry point above).
- **Stale-listing freshness.** To avoid ghost listings, the scraper stamps **"last seen,"** auto-marks a listing **stale (or pulls it)** when it disappears from the PM's site for N scans, and shows **"updated X ago"** so renters trust it. This also drives the Build-a-HAÜS teardown cleanly (below).
- **Scraper health.** Per-source health (last successful scan, error state) shows in the QSearch tab and **pings the owner when an adapter breaks**, so the feed never silently goes quiet after a PM changes their site.
- **Geofence — housing-specific, wider than events.** The scraper filters to a **housing region** that runs beyond Portland proper: the metro **plus the Willamette Valley corridor down to Salem** (north to St. Helens / Vernonia / Yacolt, south to Salem / Turner / Silverton, west to Carlton / Gaston, east to Sandy / Estacada / Scotts Mills; includes Vancouver / Camas). This is **deliberately larger than the events/directory map lock** — nightlife stays in Portland, but **people commute**, so a renter will happily live in Woodburn or Gresham and drive in. Housing keeps its *own* bounds; it does **not** reuse the tight Portland events box. The fence still stops a PM with statewide inventory from flooding the board with far-flung units.
- **Affirmative affordability badges.** Surface **"Accepts Section 8," "income-restricted," "accessible unit"** as positive badges — high value for the community and squarely *pro*-fair-housing (surfacing voucher-friendly options, never filtering people out).

Two stickiness features that make managers keep their inventory current (roadmap, not day one): **PM stats** on their page (views, saves, "N groups forming") give a manager a reason to stay and refer others, and a **"secured it" outcome loop** lets a Build-a-HAÜS mark when the group actually lands a place (even off-platform) — the best social proof, and it teaches which signals predict a good match. Operationally, the **owner's inbox needs light triage** once applications, hide-reviews, and fair-housing flags all land in it: admins can triage and annotate while the owner keeps the final add/remove call.
...
- **v0.1** — four post types (Offering a Room, Looking for Housing, Forming a HAÜS, Managed Property), one-question composer, openness flags, feed-first discovery, deep-glass HAÜS cards, request-to-chat (consent-based) via group conversations. Concierge matching. Managed Property ships with its **required safeguards** (fair-housing ingest scan, PM scam/legitimacy verification, stale-listing freshness, scraper health, metro geofence, affordability badges — see §4). No compatibility, no applications, no tours.
...
- **v0.3** — conversation prompts (soft compatibility); optional structured application + tours for households that want them; Managed Property stickiness (PM-facing stats, the Build-a-HAÜS **"secured it"** outcome loop, owner-inbox triage).
