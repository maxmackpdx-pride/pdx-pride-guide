...
- **Members** — can contribute (post links, comment, react, add dates). Join is by request or invite; requests open a conversation, they don't auto-add. **Keep "request to join" separate from "request to chat" (4.7)** — chat is a low-commitment conversation, join is asking to become a member. Two distinct actions; do not merge them.
