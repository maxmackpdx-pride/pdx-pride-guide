...
- **Verification gate.** Only accounts flagged as verified property managers can post this type; reuse the existing directory business/verification signal rather than inventing a new one. This is the one piece of *added* work — decide how a PM account gets that flag (manual admin grant is fine for v0.1).
