export const HOUSING_HOLOGRAM_MIN_ZOOM=0;
export const HOUSING_EVENT_HEIGHT_RATIO=1/3;
export const HOUSING_ICON_SCALE=.6;
export const HOUSING_ROTATION_SPEED=.08;
// Below the event artwork envelope (78.75 × 66.15), using its same distance scale.
export const housingIconSize=scale=>({width:70*Math.max(0,scale),height:58*Math.max(0,scale)});

/** Fit the projected model to CSS pixels without changing its perspective or rotation. */
export function housingScreenFit(matrix,bounds,viewport,size){
  ...
  const fit=Math.min(size.width/Math.max(.000001,(maxX-minX)*viewport.width/2),size.height/Math.max(.000001,(maxY-minY)*viewport.height/2));
  return {fit,x:(minX+maxX)/2,y:(minY+maxY)/2};
}
...
export function parseHousingHologramGlb(buffer,heightMeters=120){
  ...
  for(let i=0;i<vertices.length;i+=6)for(let axis=0;axis<3;axis++){modelBounds.min[axis]=Math.min(modelBounds.min[axis],vertices[i+axis]);modelBounds.max[axis]=Math.max(modelBounds.max[axis],vertices[i+axis]);}
