...
export const HOUSING_HOLOGRAM_MODELS={
  LOOKING:{id:'rent',url:asset('rent'),height:118,bearing:-18},
  OFFERING:{id:'rent',url:asset('rent'),height:118,bearing:16},
  FORMING:{id:'forming-hauz',url:asset('forming-hauz'),height:132,bearing:-8},
  MANAGED:{id:'hous-purple',url:asset('hous-purple'),height:126,bearing:22},
};
...
export function parseHousingHologramGlb(buffer,heightMeters=120){
  ...
  return {vertices,count:total,height:height*scale};
}
...
export function createHousingHologramLayer(maplibre,elevation=()=>0,reduced={matches:false}){
  ...
  return {
    id:'housing-holograms',type:'custom',renderingMode:'3d',instances:[],selected:null,disposed:false,
    ...
    async load(model){
      ...
      try{const buffer=await loadArrayBuffer(model.url);if(this.disposed)return;const parsed=parseHousingHologramGlb(buffer,model.height);model.vertices=parsed.vertices;model.count=parsed.count;model.dirty=true;this.map?.triggerRepaint();}
    ...
    onAdd(map,gl){
      ...
      const vertex=compile(gl.VERTEX_SHADER,`#version 300 es
        in vec3 a_position;in vec3 a_normal;uniform mat4 u_matrix;out vec3 v_normal;
        void main(){gl_Position=u_matrix*vec4(a_position,1.);v_normal=a_normal;}`);
      ...
      this.matrix=gl.getUniformLocation(this.program,'u_matrix');this.color=gl.getUniformLocation(this.program,'u_color');this.time=gl.getUniformLocation(this.program,'u_time');this.phase=gl.getUniformLocation(this.program,'u_phase');
    },
    upload(gl,model){
    ...
    render(gl,input){
      ...
      gl.uniform1f(this.time,reduced.matches?0:performance.now()/1000);
      for(const instance of ready){
        ...
        const angle=instance.definition.bearing*Math.PI/180,cos=Math.cos(angle),sin=Math.sin(angle),matrix=input.defaultProjectionData.mainMatrix,local=new Float32Array(16);
        for(let row=0;row<4;row++){
          local[row]=(matrix[row]*cos+matrix[4+row]*sin)*unit;
          local[4+row]=(-matrix[row]*sin+matrix[4+row]*cos)*unit;
          local[8+row]=matrix[8+row]*unit;
          local[12+row]=matrix[row]*origin.x+matrix[4+row]*origin.y+matrix[8+row]*base*unit+matrix[12+row];
        }
        gl.bindVertexArray(model.vao);gl.uniformMatrix4fv(this.matrix,false,local);gl.uniform3fv(this.color,rgb(instance.color));gl.uniform1f(this.phase,instance.phase);gl.drawArrays(gl.TRIANGLES,0,model.count);
      }
    ...
    onRemove(map,gl){this.disposed=true;for(const model of models.values()){if(model.buffer)gl.deleteBuffer(model.buffer);if(model.vao)gl.deleteVertexArray(model.vao);model.vertices=null;model.count=0;}gl.deleteProgram(this.program);this.instances=[];this.map=null;}
  };
}
