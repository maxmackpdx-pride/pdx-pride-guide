...
export default function OutzSelectedDetails({place}:{place:OutzDiscoveryPlace}) {
  ...
  return <div className="outz-spot-details">
    <div className="outz-spot-summary">
      ...
      <div><h3>Community rating</h3>{q.isPending?<small>Loading…</small>:q.isError?<small>Rating unavailable</small>:q.data?.rating.average!=null?<><b><Star size={16} fill="currentColor" aria-hidden="true" /> {q.data.rating.average.toFixed(1)} / 5</b><small>{q.data.rating.count} {q.data.rating.count===1?'rating':'ratings'} · OutZide</small></>:<><b>Not rated yet</b><small>OutZide community</small></>}</div>
    </div>
