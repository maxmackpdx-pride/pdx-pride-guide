...
export default function OutzNewsFeed() {
  ...
  return <section className="outz-news pdx-glass-rebind" aria-labelledby="outz-news-heading">
    ...
    {query.data?.weatherUnavailable && <p className="outz-news__notice" role="status">Weather alerts could not refresh.{query.data.weatherUpdatedAt ? ` Last checked ${stamp(query.data.weatherUpdatedAt)}.` : ""} <a href="https://www.weather.gov/alerts" target="_blank" rel="noopener noreferrer">Check the National Weather Service <ArrowUpRight size={14} aria-hidden="true" /></a></p>}
    {query.isPending ? <p role="status">Loading outdoor updates…</p> : query.isError ? <div role="status"><p>Outdoor updates couldn’t load.</p><button type="button" onClick={() => query.refetch()}>Try again</button></div> : <>
      ...
      {items.length ? <div className="outz-news__list">{items.slice(0, limit).map(item => <article key={item.id} className="outz-news__item pdx-glass-rebind" style={{ "--c": types[item.kind].color } as CSSProperties}>
        ...
        <footer><span>{item.kind === "weather" ? "National Weather Service" : item.author || "Community plans"}{item.endsAt ? ` · Ends ${stamp(item.endsAt)}` : ""}</span>
          {item.kind === "weather" ? <a href={item.href} target="_blank" rel="noopener noreferrer">Read warning <ArrowUpRight size={14} aria-hidden="true" /></a> : <Link href={item.href}>{item.kind === "checkin" ? "View plans" : item.kind === "carpool" ? "View ride" : "Open post"} <ArrowRight size={14} aria-hidden="true" /></Link>}</footer>
