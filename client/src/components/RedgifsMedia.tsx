...
export default function RedgifsMedia({ url, title }: { url: string; title: string }) {
  ...
  return <div className="z-redgifs-media">
    ...
    <a href={media.watchUrl} target="_blank" rel="noopener noreferrer" onClick={event => event.stopPropagation()}>Open on RedGIFs <ArrowUpRight size={14} aria-hidden="true" /></a>
  </div>;
}
