...
import {MoreHorizontal,CalendarDays,Check,Share2,MessageCircle,Megaphone,Heart,X} from 'lucide-react';
...
import {getMissedConnectionWindow,parsePacificDateTime} from '@shared/missedConnections';
...
type Action='options'|'rsvp'|'host'|'attendees'|'venue'|'mizzed'|null;
const dateLabel=(value:string)=>new Intl.DateTimeFormat('en-US',{timeZone:'America/Los_Angeles',weekday:'short',month:'short',day:'numeric',hour:'numeric',minute:'2-digit'}).format(new Date(parsePacificDateTime(value)??value));
...
export function RsvpRow({event,checkIn,preview,host,claimed,onOpen,onRemove,recent=false,previewContact}:{recent?:boolean;previewContact?:'host'|'venue'|'none';event:Event;checkIn?:CheckIn;preview:boolean;host:boolean;claimed:boolean;onOpen:(event:Event,target:HTMLElement)=>void;onRemove:()=>void}){
 ...
 const contacts=useQuery<{canMessageHost:boolean;canMessageVenue:boolean;venueName:string}>({queryKey:['/api/events',event.id,'message-options'],queryFn:()=>apiRequest('GET',`/api/events/${event.id}/message-options`).then(r=>r.json()),enabled:recent&&!preview&&action!==null,retry:false});
 const canMessageHost=preview?previewContact==='host':contacts.data?.canMessageHost;
 const canMessageVenue=preview?previewContact==='venue':contacts.data?.canMessageVenue;
 ...
 const open=(next:Action)=>{setError('');setBody('');setPostTitle('');if(next==='rsvp'&&!preview){setPhrase(checkIn?.message||"I'll be there");setVisibility(checkIn?.visibility||(checkIn?.isAnonymous?'anonymous':'public'));}setAction(next);};
 const mutation=useMutation({mutationFn:async(kind:'save'|'remove'|'host'|'attendees'|'venue'|'mizzed')=>{
  if(preview){if(kind!=='save'&&kind!=='remove')throw new Error('Messages are disabled in this preview.');return kind;}
  if(kind==='attendees'&&!host)throw new Error('Only the event host can post updates.');
  if(kind==='host'&&!recent&&(!event.isPublic||event.isPrivate))throw new Error('Host messaging is not available for this event.');
  ...
  if(kind==='host'||kind==='venue'||kind==='attendees')await apiRequest('POST',`/api/events/${event.id}/${kind==='attendees'?'host-messages':'message-host'}`,{body:body.trim(),...(kind==='venue'?{target:'venue'}:kind==='host'&&recent?{target:'host'}:{})});
  if(kind==='mizzed'){
   const linked=event.id>0&&getMissedConnectionWindow(event.dateStart,event.dateEnd)?.canPost;
   await apiRequest('POST','/api/missed-connections',{title:postTitle.trim()||body.trim().split('\n')[0].slice(0,80),body:body.trim(),scope:'board',...(linked?{eventId:event.id}:{eventLabel:`${event.title} · ${dateLabel(event.dateStart)}`,venueHint:event.venueName})});
  }
  return kind;
 },onSuccess:kind=>{
  ...
  if(!preview){
   ...
   if(kind==='host'||kind==='venue'||kind==='attendees')for(const path of ['/api/messages/inbox','/api/messages/sent','/api/messages/unread-count'])void queryClient.invalidateQueries({queryKey:[path]});
  }
  if(kind==='mizzed')for(const key of [['/api/missed-connections'],['/api/missed-connections/mine'],['/api/events',event.id,'missed-connections']])void queryClient.invalidateQueries({queryKey:key});
  toast({description:kind==='mizzed'?'Replies arrive privately in your inbox. Identity stays anonymous until you both reveal.':undefined,title:preview?'Preview RSVP updated':kind==='save'?'RSVP updated':kind==='remove'?'RSVP removed':kind==='host'||kind==='venue'?'Message sent':kind==='mizzed'?'Mizzed Connection posted':'Attendee update posted'});close();
 ...
 const title=action==='rsvp'?'Change RSVP':action==='host'?'Message the host':action==='attendees'?'Post message for attendees':action==='venue'?'Message venue':action==='mizzed'?'Mizzed Connection':'Event options';
 return <article className={`zaydar-rsvp-row${claimed?' is-claimed':''}`} onKeyDown={e=>{if(e.key==='Escape'&&action){e.stopPropagation();close();}}}>
  <div className="zaydar-rsvp-summary">
   <button type="button" className="zaydar-rsvp-event" onClick={e=>onOpen(event,e.currentTarget)}>
    ...
    <span><strong>{event.title}</strong><time dateTime={event.dateStart}>{dateLabel(event.dateStart)}</time><small>{event.venueName}</small>{claimed&&<em>{previewContact==='venue'?'Venue preview':previewContact==='host'||preview&&host?'Host preview':host?'You’re hosting':'Claimed event'}</em>}</span>
   </button>
  ...
  {action&&<div className="zaydar-rsvp-panel" id={`rsvp-options-${event.id}`}>
   ...
   {action==='options'?<div className="zaydar-rsvp-actions">
    {!recent&&<><button type="button" onClick={()=>open('rsvp')}><Check size={17}/>Change RSVP</button><button type="button" onClick={()=>void invite()}><Share2 size={17}/>Invite</button></>}
    {claimed&&host&&<button type="button" onClick={()=>open('attendees')}><Megaphone size={17}/>Post message for attendees</button>}
    {!recent&&event.isPublic&&!event.isPrivate&&!host&&<button type="button" onClick={()=>open('host')}><MessageCircle size={17}/>Message the host</button>}
    {recent&&<>
     {!preview&&contacts.isLoading&&<p role="status">Checking venue and host…</p>}
     {!preview&&contacts.isError&&<p className="zaydar-rsvp-note">Venue and host contacts unavailable. <button type="button" onClick={()=>void contacts.refetch()}>Retry</button></p>}
     {canMessageVenue&&<button type="button" onClick={()=>open('venue')}><MessageCircle size={17}/>Message venue</button>}
     {canMessageHost&&<button type="button" onClick={()=>open('host')}><MessageCircle size={17}/>Message the host</button>}
     <button type="button" onClick={()=>open('mizzed')}><Heart size={17}/>Mizzed Connection</button>
    </>}
   ...
   </form>:<form onSubmit={e=>{e.preventDefault();mutation.mutate(action as 'host'|'venue'|'attendees'|'mizzed');}}>
    <p>{action==='attendees'?'This update appears on the event and notifies its attendees. Replies go to your inbox.':action==='mizzed'?'Post a Mizzed Connection for this event. Replies arrive privately in your inbox; identity stays anonymous until you both reveal.':`Send a private message to ${action==='venue'?(contacts.data?.venueName||event.venueName):'the event host'}. Replies go to your inbox.`}</p>
    {action==='mizzed'&&<label>Title (optional)<input value={postTitle} onChange={e=>setPostTitle(e.target.value)} maxLength={80} placeholder="A moment you remember…"/></label>}
    <label>Message<textarea autoFocus rows={3} maxLength={action==='mizzed'?500:1000} value={body} onChange={e=>setBody(e.target.value)} placeholder={action==='attendees'?'An update for your attendees…':action==='mizzed'?'Where did you cross paths?':action==='venue'?'Ask the venue…':'Ask the host…'}/></label>
    {preview&&<p className="zaydar-rsvp-note">Preview only. Messages won’t be sent.</p>}
    <button className="zaydar-rsvp-submit" type="submit" disabled={preview||mutation.isPending||!body.trim()}>{mutation.isPending?'Sending…':action==='attendees'?'Post message':action==='mizzed'?'Post Mizzed Connection':'Send message'}</button>
   </form>}
