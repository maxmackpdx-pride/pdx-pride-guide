...
export default function SafetyGuide({ context, compact = false, onReport, onBlock }: Props) {
  ...
  useEffect(() => {
    const key = `zay_report_seen:${context}`;
    try {
      if (sessionStorage.getItem(key)) return;
      sessionStorage.setItem(key, "1");
    } catch { /* still record when storage is unavailable */ }
    trackProductEvent("report_control_seen", context);
  }, [context]);
  const reportOpened = () => {
    trackProductEvent("report_opened", context);
    onReport?.();
  };
  return (
