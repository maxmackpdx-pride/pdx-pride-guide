...
Public DNS checked on August 3, 2026 establishes two separate email systems. Zaylist's human mailbox is Microsoft 365 / Exchange Online, administered or provisioned through GoDaddy. Zaylist's scalable no-reply and transactional delivery service is Resend: `resend._domainkey.zaylist.com` publishes Resend's DKIM key, while `send.zaylist.com` publishes Amazon SES feedback MX and SPF records used by the delivery layer. Tucker reports that Resend was selected as a free service with a path to paid scaling. The current Resend plan, allowance, billing state, configured sender addresses, forwarding behavior, API integration, and production send status still require authenticated account or application evidence.
