...
export function EventCard({
  ...
  return (
    <Tag className={`pdxRow pdx-glass-rebind ${className}`} href={href}
      style={{ "--_day": base, "--_c": base, "--c": base, "--dc": base, ...style }} {...rest}>
      <span className="pdxRow__refract" aria-hidden="true" />
      ...
      <div className="pdxRow__aside">
        ...
        {showClaim && (claimPending
          ? <span className="pdxRow__claim pdxRow__claim--pending">Claim pending</span>
          : <button type="button" className="pdxRow__claim"
              onClick={(ev) => { stop(ev); onClaimClick && onClaimClick(); }}>Claim this event &#8594;</button>
        )}
      </div>
    </Tag>
  );
