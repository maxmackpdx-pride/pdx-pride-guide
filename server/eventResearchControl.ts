...
import { QSEARCH_REQUIRED_FACTS, qsearchCreateDefaults, qsearchFieldNeedsEvidence } from "@shared/qsearchPublishPolicy";
...
export function evaluateDecisionGate(input: {
  ...
  const requiredEvidenceFields = fields.filter(field => qsearchFieldNeedsEvidence(field, expectedValues[field]));
  const latest = (field: string) => evidence.find(item => item.field === field && item.authority_level === "primary");
  const missingKnown = requiredEvidenceFields.filter(field => !latest(field));
  const mismatched = requiredEvidenceFields.filter(field => latest(field) && JSON.stringify(parseJson(latest(field)!.observed_value_json, null)) !== JSON.stringify(expectedValues[field]));
  const conflicted = fields.filter(field => conflicts.some(item => item.field === field));
  const insufficientIndependent: string[] = [];
  ...
  const publishable = !missingKnown.length && !mismatched.length;
  return {
    ok: true as const,
    decision: publishable ? "approved" : "blocked",
    publishable,
    missingEvidence: missingKnown,
...
/** New candidates need current evidence for the three required facts. Optional
 * facts can remain unverified; an older disagreement stays in the audit log. */
...
function evaluateCandidateDecisionGate(input: {
  ...
  if (!input.proposedValues || typeof input.proposedValues !== "object" || Array.isArray(input.proposedValues) || !Array.isArray(input.fields) || !input.fields.length) {
    return { ok: false as const, status: 400, error: "candidate gate requires proposedValues and fields" };
  }
  const values = qsearchCreateDefaults(input.proposedValues);
  ...
  const required = [...QSEARCH_REQUIRED_FACTS];
  ...
  if (!["ALL_AGES", "18_PLUS", "21_PLUS", "UNVERIFIED"].includes(String(values.ageRequirement))) invalidCandidateFields.push("ageRequirement");
  if (!["FREE", "TICKETED", "DOOR_FEE", "SUGGESTED_DONATION", "UNKNOWN"].includes(String(values.admission))) invalidCandidateFields.push("admission");
  const start = parsePacificDateTime(String(values.dateStart));
  const end = parsePacificDateTime(String(values.dateEnd || ""));
  if (start == null) invalidCandidateFields.push("dateStart");
  if (values.dateEnd && (end == null || start == null || end <= start)) invalidCandidateFields.push("dateEnd");
  if (values.status === "LIVE" && (end != null ? end <= Date.now() : (pacificCalendarDate(String(values.dateStart)) || "") < pacificTodayDate())) invalidCandidateFields.push("pastEvent");
  const cutoff = new Date(Date.now() - 30 * 24 * 3600_000).toISOString();
  ...
  `).all(candidateKey, input.runId, cutoff) as Array<any>).map(item => ({ ...item, field: evidenceField(item.field) }));
  // The most recent primary observation wins for each field. Older conflicting
  // observations remain recorded but no longer veto a current official update.
  const latestByField = new Map<string, (typeof observations)[number]>();
  for (const observation of observations) {
    if (observation.authority_level === "primary" && !latestByField.has(observation.field)) latestByField.set(observation.field, observation);
  }
  const fieldsNeedingEvidence = fields.filter(field => qsearchFieldNeedsEvidence(field, values[field]));
  const missingEvidence = fieldsNeedingEvidence.filter(field => !latestByField.has(field));
  const mismatchedEvidence = fieldsNeedingEvidence.filter(field => latestByField.has(field) && JSON.stringify(parseJson(latestByField.get(field)!.observed_value_json, null)) !== JSON.stringify(values[field]));
  const latestStatus = latestByField.get("status");
  const canceled = values.status === "LIVE" && latestStatus && ["CANCELED", "CANCELLED", "POSTPONED"].includes(String(parseJson(latestStatus.observed_value_json, "")).toUpperCase());
  const conflictingEvidence = observations.filter(item => item.authority_level === "primary" && latestByField.has(item.field) && JSON.stringify(parseJson(item.observed_value_json, null)) !== JSON.stringify(parseJson(latestByField.get(item.field)!.observed_value_json, null))).map(item => item.field);
  const insufficientIndependentVerification: string[] = [];
  ...
  const publishable = !missingRequiredFields.length && !invalidCandidateFields.length && !missingEvidence.length && !mismatchedEvidence.length && !canceled && !duplicate;
  return {
    ok: true as const, candidateKey, decision: publishable ? "approved" : "blocked", publishable,
    ...
    evidenceReceiptIds: [...latestByField.values()].filter(item => fields.includes(item.field)).map(item => item.id),
  };
}
